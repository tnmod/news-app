...
const EditNews = (props) => {
  ...
  useEffect(() => {
    if (dataNews.image != undefined) {
      setUriImage(dataNews.image);
    }
    setContent(dataNews.content);
    setTitle(dataNews.title);
  }, [])
  ...
  const uploadNews = async () => {
    ...
    if (getUriImage == '') {
      ...
      if (!upload.error) {
        const reponse = await AxiosIntance().put('/articles/' + dataNews._id + '/update', { title: getTitle, content: getContent, image: upload.data.path });
        if (!reponse.error) {
          navigation.goBack();
          setAlertDialog(0);
        }
      ...
      const reponse = await AxiosIntance().put('/articles/' + dataNews._id + '/update', { title: getTitle, content: getContent, image: getUriImage });
      if (!reponse.error) {
        navigation.goBack();
        setAlertDialog(0);
      }
    ...
    <View style={[{ backgroundColor: '#fff', flex: 1 }]} >
      ...
      <View style={[styles.container]} >
        ...
        <View style={[styles.containerTitle, { marginTop: 16, marginBottom: 4 }]} >
          <TextInput placeholderTextColor='#A0A3BD' defaultValue={dataNews.title} onChangeText={Text => setTitle(Text)} multiline style={[styles.textimg, { color: '#050505', fontSize: 24, lineHeight: 36, minHeight: 36, padding: 0 }]} placeholder='News title' />
        </View>
        <View style={[{ width: '100%', height: 1.2, backgroundColor: '#C4C4C4' }]} ></View>
        <View style={[styles.containerTitle, { flex: 1, }]} >
          <TextInput placeholderTextColor='#A0A3BD' defaultValue={dataNews.content} onChangeText={Text => setContent(Text)} multiline style={[styles.textimg, { color: '#4E4B66', fontSize: 16, lineHeight: 24, }]} placeholder='Add News/Article' />
        </View>
        <View style={{ backgroundColor: '#f6f6f6', width: '200%', height: 1 }}></View>
